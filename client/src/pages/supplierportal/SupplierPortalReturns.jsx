...
export default function SupplierPortalReturns() {
  ...
  return (
    <div>
      <div className="mb-6">
        ...
        <p className="text-[13px] text-muted-foreground mt-1">Leads that were returned to you, with the reason given.</p>
      </div>
        ...
        <table className="w-full text-[13px] min-w-[720px]">
          <thead>
            <tr className="border-b border-border bg-muted/50">
              {['Lead', 'Reason', 'Status', 'Requested', 'Resolved', 'Notes'].map(h => (
          ...
          <tbody className="divide-y divide-border">
            {returns.length === 0 && (
              <tr><td colSpan={6} className="px-4 py-8 text-center text-muted-foreground">No returns yet.</td></tr>
            )}
            {returns.map(r => {
