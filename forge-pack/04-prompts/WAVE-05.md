...
Real traffic, commercial sending disabled. Verify that shadow mode truly has no commercial side effects before running it rather than assuming it from the function name. Every discrepancy against expected LeadByte behaviour is explained or fixed.

## Write ownership

Owns docs/SHADOW-REPORT.md only. No source changes in this wave; the freeze is in effect.
